...
def deletar_insumo():
    if len(nome_insumo.get()) < 2:
        ...
        # escrever na caixa de texto
        caixa_texto.insert("1.0", f"{nome_insumo.get()} invalido !")
        return
...
def consumir_insumo():
    if len(nome_insumo.get()) < 2 or len(lote_insumo.get()) < 1:
        ...
        # escrever na caixa de texto
        caixa_texto.insert("1.0", f"Nome e lote Inválido!")
        return
    ...
    # escrever na caixa de texto
    caixa_texto.insert("1.0", f"{nome_insumo.get()} foi consumido em {qtde_insumo.get()} unidade(s)!")
